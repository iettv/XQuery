...
declare function local:createElementRangeIndex($config as element(configuration), $server-config as element(http-server), $group-name as xs:string)
{
	...
	let $Transcript := let $Index := admin:database-range-element-attribute-index("string", "", "Transcript", "", "active",  "http://marklogic.com/collation/", fn:false() ) return admin:save-configuration(admin:database-add-range-element-attribute-index($config, $dbid, $Index))
	let $Subtitle := let $Index := admin:database-range-element-attribute-index("string", "", "Subtitle", "", "active",  "http://marklogic.com/collation/", fn:false() ) return admin:save-configuration(admin:database-add-range-element-attribute-index($config, $dbid, $Index))
	return "DONE"
};
