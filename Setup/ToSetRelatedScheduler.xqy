...
declare function local:createServerTask($config,$task-path,$task-root,$task-period,$task-minute, $task-database,$task-modules,$task-user)
{
	...
	let $task    := admin:group-daily-scheduled-task($task-path, $task-root,1,xs:time("24:00:00"),xdmp:database($task-database),xdmp:database($task-modules), xdmp:user($task-user), (), "higher")
...
(: Scheduler setup :)
let $task-path := "/Scheduler/getRelatedContent.xqy"
let $task-root :="/"
let $task-period:=1
let $task-minute :=00
...
return "Related Scheduler created"