...
let $WebDavPort := 9505
let $HttpPort := 9506
let $XDBCService := "IETTV-Activity-XDBC-Server"
let $WebDavService := "IETTV-Activity-WEBDAV-Server"
...
let $task-path := "/Scheduler/ChannelMostPopular.xqy"
let $task-root :="/"
let $task-period:=2
let $task-minute :=00
let $task-database :="IETTV-Activity-Database"
let $task-modules:="IETTV-Activity-Modules-Database"
let $task-user:="admin"
...
(: TO DO - To be moved in function with all required params and called :)
let $config  := admin:get-configuration()
...
return "Activity Database, Servers created Successfully"