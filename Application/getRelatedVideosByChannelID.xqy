...
(:~ This service is to generate Related Video content and depends on external parameters
	...
	: @ChannelID   	- Related Videos will be fetched from given ChannelID based Popular and Latest List
	...
	:
	:  <Videos><SkipChannel><ID>1</ID><ID>2</ID><ID>3</ID></SkipChannel><VideoID>017e91cb-c125-4bca-af34-9e11ffda7186</VideoID><Channels><ChannelID>7167</ChannelID><ChannelID>7187</ChannelID></Channels></Videos> 
:)
...
let $Channels 		:= $InputXML/Videos/Channels
...
return
	...
	then
		(
			xdmp:log("[ RelatedVideo ][ ERROR ][  ChannelID's is Empty ]"),
			"ERROR: Please provide ChannelID's. Currently it is empty."
		)
	else
	if($VideoID)
	then
		...
		let $LatestVideoList 	:= 	for $eachChannel in $Channels/ChannelID
									for $eachVideoID in fn:doc(concat($constants:ChannelLatest,$eachChannel/text(),".xml"))/Latest/VideoID/text() 
									return replace($eachVideoID,"-","")
		...
		let $CurrentMostPopularList := 	for $eachChannel in $Channels/ChannelID
										for $eachVideoID in CHANNEL:GetChannelMostPopularFile($eachChannel/text())/ChannelMostPopular /Video/VideoID/text() 
										return replace($eachVideoID,"-","")
