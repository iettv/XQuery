...
(: <Video><UserID></UserID><TopList>10</TopList><SearchKey>LocationName</SearchKey><SearchValue>*stevenage*</SearchValue><VideoType>NonPromo</VideoType>
   <PageLength>10</PageLength><StartPage>1</StartPage><IsAbused>yes|no</IsAbused></Video>
 :)

declare function local:getVideoChunk($EachVideo,$IsAbused)
{
	if( $IsAbused = 'yes' )
	then
		...
		return
			if( $IsAbuseReported=fn:true() )
			then
				<Video ID="{$VideoID}" comment="{if($IsCommentAvailable=fn:true()) then 'yes' else 'no'}" abuse="{if($IsAbuseReported=fn:true()) then 'yes' else 'no'}">
				{
...
let $Log 		:= xdmp:log("[ GetAdminVideoByCriteria ][ CALL2222 ][ Service call successfully ]")
...
let $IsAbused    := $InputXML/Video/IsAbused
	...
	<Videos>
		{
			...
			return 
				...
				then
				(
					let $FilteredResult := 	if($PageLength!='0' and $IsAbused!='yes')
											then
												for $EachVideo in ($Result)[position() ge $Start and position() lt $End]
												return local:getVideoChunk($EachVideo,$IsAbused)  
											else
												for $EachVideo in $Result
												return local:getVideoChunk($EachVideo,$IsAbused)
					return
						(<Count>{if($IsAbused!='yes') then count($Result) else count($FilteredResult)}</Count>, if($IsAbused!='yes') then $FilteredResult else $FilteredResult[position() ge $Start and position() lt $End])
				)
				else
					"NONE"
		}
