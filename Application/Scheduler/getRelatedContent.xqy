...
let $Log := xdmp:log("[ IET-TV ][ Related Content Match ][ Call ][ ======================= Video Related Content Matching Process Start ======================= ]")
for $VideoXml in fn:collection("PublishedCopy")
let $VideoUri     			:= xdmp:node-uri($VideoXml)
let $VideoID                := fn:data($VideoXml/Video/@ID)
let $VideoTitle   			:= $VideoXml/Video/BasicInfo/Title
(:let $VideoDefaultChannelID 	:= fn:data($VideoXml/Video/BasicInfo/ChannelMapping/Channel[@default='true']/@ID)
let $VideoKeyWordInfo      	:= $VideoXml/Video/KeyWordInfo/ChannelKeywordList/Channel[@ID=$VideoDefaultChannelID]/KeywordList:)
		...
		let $RealtedInfo 	:= xdmp:eval(	"xquery version '1.0-ml';
											...
											return
											  let $FilteredElement := for $EachResult in $MatchingResults/MatchingKeys
																	  return
																		<Record>
																			<MatchedUri>{$EachResult/FileUri/text()}</MatchedUri>
																			<Title>{$EachResult/Original/Title/text()}</Title>
																			<DOI>{$EachResult/DOI/text()}</DOI>
																			<Type>{$EachResult/Type/text()}</Type>
																		</Record>
		 ...
		 return
		 xdmp:document-insert(fn:concat("/Related/",$VideoID,".xml"),$RealtedInfo,(), $constants:COL_RELATED)
	 ,
		xdmp:log("[ IET-TV ][ Related Content Match ][ End ][ ======================= Video Related Content Matching Process End ======================= ]")
	 )