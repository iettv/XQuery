    ...
    then
      ...
      return
        if( $doc/Video/PublishInfo/VideoPublish[@active="yes"] )
          then
                 if ( not($doc/Video/PublishInfo/VideoPublish[@active="yes"][DOI]) )
                 then xdmp:node-insert-after($doc/Video/PublishInfo/VideoPublish/URL, <DOI>{$updatedDoi}</DOI>)
                 else
                 if ( $doc/Video/PublishInfo/VideoPublish[@active="yes"][DOI] )
                 then xdmp:node-replace($doc/Video/PublishInfo/VideoPublish/DOI, <DOI>{$updatedDoi}</DOI>)
                 else xdmp:log(("======DOI not inserted =============", $VideoURI))
          else
          if( $doc/Video/PublishInfo/LivePublish[@active="yes"] )
          then
                 if ( not($doc/Video/PublishInfo/LivePublish[@active="yes"][DOI]) )
                 then xdmp:node-insert-after($doc/Video/PublishInfo/LivePublish/URL, <DOI>{$updatedDoi}</DOI>)
                 else
                 if ( $doc/Video/PublishInfo/LivePublish[@active="yes"][DOI] )
                 then xdmp:node-replace($doc/Video/PublishInfo/LivePublish/DOI, <DOI>{$updatedDoi}</DOI>)
                 else xdmp:log(("======DOI not replaced =============", $VideoURI))
           else xdmp:log(("======DOI not inserted/replaced =============", $VideoURI))
    else ()
    ,
    ...
    then
      ...
      return
        ...
        then
               ...
               if ( $doc/Video/PublishInfo/VideoPublish[@active="yes"][DOI] )
               then xdmp:node-replace($doc/Video/PublishInfo/VideoPublish/DOI, <DOI>{$updatedDoi}</DOI>)
               else xdmp:log(("======DOI not inserted =============", $PHistoryUri))
        else
        if( $doc/Video/PublishInfo/LivePublish[@active="yes"] )
        then
               if ( not($doc/Video/PublishInfo/LivePublish[@active="yes"][DOI]) )
               ...
               if ( $doc/Video/PublishInfo/LivePublish[@active="yes"][DOI] )
               then xdmp:node-replace($doc/Video/PublishInfo/LivePublish/DOI, <DOI>{$updatedDoi}</DOI>)
               else xdmp:log(("======DOI not replaced=============", $PHistoryUri))
         else xdmp:log(("======DOI not inserted/replaced=============", $PHistoryUri))
     else ()
)