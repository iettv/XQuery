...
declare function local:createElementRangeIndex($config as element(configuration), $server-config as element(http-server), $group-name as xs:string)
{
	...
	let $Series := let $Index := admin:database-range-element-attribute-index("string", "", "Series", "", "ID",  "http://marklogic.com/collation/", fn:false() ) return admin:save-configuration(admin:database-add-range-element-attribute-index($config, $dbid, $Index))
	let $Event := let $Index := admin:database-range-element-attribute-index("string", "", "Event", "", "ID",  "http://marklogic.com/collation/", fn:false() ) return admin:save-configuration(admin:database-add-range-element-attribute-index($config, $dbid, $Index))
	return "DONE"
};
